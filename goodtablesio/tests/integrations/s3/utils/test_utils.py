...
def test_set_up_bucket_s3_add_policy_access_denied():
    ...
    with mock.patch('goodtablesio.integrations.s3.utils._check_connection'), \
...
def test_set_up_bucket_lambda_add_permission_fails():
    ...
    with mock.patch('goodtablesio.integrations.s3.utils._check_connection'), \
            mock.patch('goodtablesio.integrations.s3.utils._add_policy'), \
...
def test_set_up_bucket_lambda_add_notification_fails():
    ...
    with mock.patch('goodtablesio.integrations.s3.utils._check_connection'), \
            mock.patch('goodtablesio.integrations.s3.utils._add_policy'), \
            mock.patch('goodtablesio.integrations.s3.utils._add_lambda_permission'), \
...
def mock_lambda_client():
    mock_client = mock.patch('goodtablesio.integrations.s3.utils.LambdaClient')
    mock_client.start()
    yield mock_client
    mock_client.stop()
...
@pytest.fixture
def mock_s3_client():
    mock_client = mock.patch('goodtablesio.integrations.s3.utils.S3Client')
    mock_client.start()
    yield mock_client
    mock_client.stop()
